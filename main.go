...
defer func() {
if r := recover(); r != nil {
// Stampalo su stderr: se lanci da terminale lo vedrai
...
// Verifica FFmpeg all'avvio, ma FALLBACK sicuro: mostra dialog dopo che la GUI è pronta
go func() {
	// piccolo delay per garantire che la finestra sia pronta
	time.Sleep(200 * time.Millisecond)
	ffmpegPath := findFFmpeg()
	if ffmpegPath == "" {
		// usa Invoke per chiamare UI dal thread corretto
...
go func() {
	...
	s.isRunning = false
	// Aggiornamento UI: usa Canvas.Invoke per essere sicuri del thread
...
// prova terminazione gentile, poi kill
...
// NOTE: non forziamo la gestione di SysProcAttr qui per evitare problemi di build multipiattaforma.
// Se vuoi nascondere la console su Windows possiamo riattivare utils_windows.go + stub per non-windows.
